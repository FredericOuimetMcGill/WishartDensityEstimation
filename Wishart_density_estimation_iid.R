...
XX <- function(j, n) { 
  res <- switch(as.character(j),
                ...
                "3" = {
                  lapply(1:n, function(x) {
                    if (runif(1) < 0.5) {
                      LaplacesDemon::rinvwishart(4, S1)
                    } else {
                ...
                "4" = {
                  lapply(1:n, function(x) {
                    if (runif(1) < 0.5) {
                      LaplacesDemon::rinvwishart(5, S3)
                    } else {
                      LaplacesDemon::rinvwishart(6, S4)
                    }
                  })
                },
                "5" = {
                ...
                "6" = {
                  lapply(1:n, function(x) {
                    X <- matrixsampling::rmatrixbeta(1, d, 3, 10, def = 2, checkSymmetry = TRUE)[,,1]
                    symmetrize(as.matrix(solve(diag(1, d) - X) %*% X))
                  })
                },
                {
...
f <- function(j, X) { # X is an SPD matrix of size d x d
  if (j == 1) {
    ...
    # Case when j = 3
    res <- 0.5 * LaplacesDemon::dinvwishart(X, 4, S1) + 0.5 * LaplacesDemon::dinvwishart(X, 5, S2)
  } else if (j == 4) {
    # Case when j = 4
    res <- 0.5 * LaplacesDemon::dinvwishart(X, 5, S3) + 0.5 * LaplacesDemon::dinvwishart(X, 6, S4)
  } else if (j == 5) {
    # Case when j = 5
    res <- dmatrixbeta_typeII(X, 2, 4)
  } else if (j == 6) {
    # Case when j = 6
    res <- dmatrixbeta_typeII(X, 3, 10)
  } else {
...
#     result <- ISE(XX_sample, j, method, tol_test)
#     
#     # Store the result in the appropriate column
#     if (method == "WK") {
#       results_matrix[JJ == j, 2] <- result
